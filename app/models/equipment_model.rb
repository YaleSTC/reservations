class EquipmentModel < ActiveRecord::Base
  ...
  def available_count(date)
    ...
    reserved_count = Reservation.find(:all, :conditions => ["checked_in IS NULL and checked_out IS NULL and equipment_model_id = ? and start_date <= ? and due_date >= ?", self.id, date.to_time.utc, date.to_time.utc]).size
    overdue_count = Reservation.find(:all, :conditions => ["checked_in IS NULL and checked_out IS NOT NULL and equipment_model_id = ? and due_date <= ?", self.id, Date.today.to_time.utc]).size
