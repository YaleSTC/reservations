class EquipmentModel < ActiveRecord::Base
  ...
  def not_associated_with_self
    unless associated_equipment_models.where(id: id).blank?
      errors.add(:associated_equipment_models, 'You cannot associate a model with itself. Please deselect ' + name)
    end
  end
  ...
  def num_reserved(start_date, due_date, source_reservations)
    ...
    start_date.to_date.upto(due_date.to_date) do |d|
      reserved = Reservation.number_for_model_on_date(d, id, source_reservations)
