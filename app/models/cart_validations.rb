module CartValidations
  ...
  def validate_all(renew = false)
    ...
    source_res = Reservation.not_returned.where(equipment_model_id: items.keys).reserved_in_date_range(start_date, due_date).all
  ...
  def check_start_date_blackout
    ...
    if Blackout.hard.for_date(start_date).count > 0
      errors << "#{Blackout.get_notices_for_date(start_date, :hard)} (a reservation cannot start on #{start_date.to_date.strftime('%m/%d')})"
    end
    errors
  end
  ...
  def check_due_date_blackout
    ...
    if Blackout.hard.for_date(due_date).count > 0
      errors << "#{Blackout.get_notices_for_date(due_date, :hard)} (a reservation cannot end on #{due_date.to_date.strftime('%m/%d')})"
    end
    errors
  end
  ...
  def check_max_items
    ...
    # 4 queries
    errors = []
    relevant = Reservation.for_reserver(reserver_id).not_returned.includes(:equipment_model).all
    category = {}
  ...
  def check_availability(model = EquipmentModel.find(items.keys.first),
    ...
    errors = []
    if model.num_available_from_source(start_date, due_date, source_res) < quantity
  ...
  def check_should_be_renewed(user_reservations = Reservations.for_reserver(reserver).not_returned,
  ...
  def check_requirements(items = get_items)
    ...
    ["#{user.name} is missing the following certifications: #{unfulfilled_req_text.to_sentence}"]
  end
end
