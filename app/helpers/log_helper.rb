module LogHelper
  ...
  def transform_attributes(arr)
    ...
    val_dict = { 'reservation_id' => proc { |id| get_reservation_link(id) },
                 'reserver_id' => proc { |id| get_user_link(id) },
                 'checkout_handler_id' => proc { |id| get_user_link(id) },
                 'checkin_handler_id' => proc { |id| get_user_link(id) },
                 'equipment_model_id' => proc { |id| get_model_link(id) },
                 'equipment_object_id' => proc { |id| get_object_link(id) },
                 'start_date' => proc { |date| get_day(date) },
                 'due_date' => proc { |date| get_day(date) },
                 'checked_out' => proc { |date| get_time(date) },
                 'checked_in' => proc { |date| get_time(date) },
                 'created_at' => proc { |date| get_time(date) },
                 'updated_at' => proc { |date| get_time(date) }
              }

    if arr[1].nil?
      val = 'N/A'
    elsif val_dict.key? arr[0]
