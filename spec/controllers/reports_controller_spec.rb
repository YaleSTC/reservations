  ...
  context 'with admin user' do
    ...
    describe 'GET index' do
      ...
      it 'builds the tables correctly' do
        reservations = Reservation.starts_on_days(assigns(:start_date),
                                                  assigns(:end_date))
        expect(Report).to receive(:build_new).with(:equipment_model_id, reservations)
    ...
    describe 'GET subreport' do
      ...
      it 'builds the proper subreports' do
        ...
        expect(Report).to receive(:build_new).with(:equipment_model_id, reservations, MODEL_COLUMNS)
        expect(Report).to receive(:build_new).with(:equipment_object_id, reservations, MODEL_COLUMNS)
        expect(Report).to receive(:build_new).with(:reserver_id, reservations, MODEL_COLUMNS)
        expect(Report).to receive(:build_new).with(:id, reservations, RES_COLUMNS)
        get :subreport, class: 'equipment_model', id: 1
