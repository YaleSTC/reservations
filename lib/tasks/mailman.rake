...
task :send_upcoming_checkin_reminder => :environment do
  if AppConfig.first.upcoming_checkin_email_active?
    ...
    upcoming_reservations = Reservation.where("checked_out IS NOT NULL and checked_in IS NULL and due_date >= ? and due_date < ?", Time.now.midnight.utc, Time.now.midnight.utc + 1.day)
...
task :send_overdue_checkin_reminder => :environment do
  if AppConfig.first.overdue_checkin_email_active?
    ...
    overdue_reservations = Reservation.where("checked_out IS NOT NULL and checked_in IS NULL and due_date < ?", Time.now.midnight.utc)
...
task :send_reservation_notes => :environment do
  ...
  notes_reservations_out = Reservation.where("notes IS NOT NULL and checked_out IS NOT NULL and checked_in IS NULL and notes_unsent = true")
  notes_reservations_in = Reservation.where("notes IS NOT NULL and checked_out IS NOT NULL and checked_in IS NOT NULL and notes_unsent = true")
...
task :send_overdue_checked_in_fine => :environment do
  ...
  overdue_checked_in.each do |overdue_checked_in|
    UserMailer.overdue_checked_in_fine(overdue_checked_in).deliver
  end
  puts "Done!"
end
